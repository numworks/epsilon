...
int VerticalOffsetLayoutNode::writeTextInBuffer(char * buffer, int bufferSize, int numberOfSignificantDigits) const {
  if (m_type == Type::Subscript) {
    if (bufferSize == 0) {
      return -1;
    }
    ...
    numberOfChar += const_cast<VerticalOffsetLayoutNode *>(this)->indiceLayout()->writeTextInBuffer(buffer+numberOfChar, bufferSize-numberOfChar, numberOfSignificantDigits);
    if (numberOfChar >= bufferSize-1) { return bufferSize-1; }
  ...
  int numberOfChar = LayoutEngine::writePrefixSerializableRefTextInBuffer(SerializableRef(const_cast<VerticalOffsetLayoutNode *>(this)), buffer, bufferSize, numberOfSignificantDigits, "^");
  if (numberOfChar >= bufferSize-1) { return bufferSize-1; }
