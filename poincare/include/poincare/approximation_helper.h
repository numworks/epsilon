...
namespace ApproximationHelper {
  ...
  template<typename T> Evaluation<T> Map(const ExpressionNode * expression, ExpressionNode::ApproximationContext approximationContext, ComplexCompute<T> compute, bool mapOnMatrix = false, bool mapOnList = true);
