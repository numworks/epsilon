...
StorageCalculationGraphController::StorageCalculationGraphController(Responder * parentResponder, StorageGraphView * graphView, BannerView * bannerView, Shared::InteractiveCurveViewRange * curveViewRange, CurveViewCursor * cursor, I18n::Message defaultMessage) :
...
View * StorageCalculationGraphController::view() {
  return m_graphView;
}

void StorageCalculationGraphController::viewWillAppear() {
...
bool StorageCalculationGraphController::handleEvent(Ion::Events::Event event) {
...
void StorageCalculationGraphController::setFunction(StorageCartesianFunction * function) {
  m_graphView->selectFunction(function);
  m_function = *function;
}

void StorageCalculationGraphController::reloadBannerView() {
  m_bannerView->setNumberOfSubviews(2);
  reloadBannerViewForCursorOnFunction(m_cursor, &m_function, 'x');
}

bool StorageCalculationGraphController::moveCursor(int direction) {
...
Expression::Coordinate2D StorageCalculationGraphController::computeNewPointOfInteresetFromAbscissa(double start, int direction) {
