...
int main(int argc, char* argv[]) {
  ...
  computeMetrics(face, &maxWidth, &maxAboveBaseline, &maxBelowBaseline,
                 glyphWidths);
  ENSURE(maxWidth > 0 && maxAboveBaseline > 0 && maxBelowBaseline > 0,
         "Computing bounds");
  ...
  checkGlyphDimensions(glyphWidth, packedGlyphWidth, glyphHeight,
                       packedGlyphHeight);

  int glyphDataOffsetLength = 0, glyphDataLength = 0;
  uint16_t* glyphDataOffset =
      (uint16_t*)malloc((NumberOfCodePoints + 1) * sizeof(uint16_t*));
  int sizeOfUncompressedGlyphBuffer =
      glyphWidth * glyphHeight * k_grayscaleBitsPerPixel / 8;
  ...
  int numberOfCodePointsPairs = writeCodePointToGlyphTableFiles(
      sharedFontPath, k_grayscaleBitsPerPixel, sizeOfUncompressedGlyphBuffer);
  generateGlyphData(face, glyphDataOffset, &glyphDataOffsetLength, glyphData,
                    glyphDataBufferSize, &glyphDataLength, glyphWidth,
                    glyphHeight, maxAboveBaseline, k_grayscaleBitsPerPixel);
  writeFontHeaderFile(outputH, fontName, glyphWidth, glyphHeight);
  writeFontSourceFile(outputCpp, fontName, glyphWidth, glyphHeight,
                      glyphDataOffset, glyphDataOffsetLength, glyphData,
                      glyphDataLength, glyphWidths, glyphWidthsLength,
                      k_grayscaleBitsPerPixel, numberOfCodePointsPairs);

#if GENERATE_PNG
  storeRenderedGlyphsImage(outputPng, face, glyphWidth, glyphHeight,
                           maxAboveBaseline);
#endif
...
void checkInputs(int argc, char* argv[]) {
  ...
  if (argc != expectedNumberOfArguments) {
#ifdef GENERATE_PNG
    fprintf(stderr,
            "Usage: %s fontFile requestedGlyphWidth requestedGlyphHeight "
            "packedGlyphWidth packedGlyphHeight fontName outputH outputCpp "
            "sharedFontPath outputPng\n",
            argv[0]);
#else
    fprintf(stderr,
            "Usage: %s fontFile requestedGlyphWidth requestedGlyphHeight "
    ...
    fprintf(stderr,
            "  requestedGlyphWidth: Width of bitmap glyphs, in pixels\n");
    fprintf(stderr,
            "  requestedGlyphHeight: Height of bitmap glyphs, in pixels\n");
    fprintf(stderr,
            "  packedGlyphWidth: Minimal glyph width in pixels. Pass 0 if "
            "unsure.\n");
    fprintf(stderr,
            "  packedGlyphHeight: Minimal glyph height in pixels. Pass 0 if "
            "unsure.\n");
    ...
    fprintf(stderr,
            "  sharedFontPath: Name of the generated shared C codepoint to "
            "glyph index source file\n");
#ifdef GENERATE_PNG
  ...
  for (int i = 0; i < NumberOfCodePoints - 1; i++) {
    ENSURE(CodePoints[i] < CodePoints[i + 1],
           "CodePoints array must be sorted: %d > %d", CodePoints[i],
           CodePoints[i + 1]);
  }
}

void initTTF(FT_Library* library, FT_Face* face, const char* fontFile,
  ...
  ENSURE(!FT_New_Face(*library, fontFile, 0, face), "Loading font file %s",
         fontFile);
  ENSURE(!FT_Set_Pixel_Sizes(*face, requestedGlyphWidth, requestedGlyphHeight),
         "Setting face pixel size to %dx%d", requestedGlyphWidth,
         requestedGlyphHeight);
}

void computeMetrics(FT_Face face, int* maxWidth, int* maxAboveBaseline,
                    int* maxBelowBaseline, uint8_t* glyphWidths) {
...
void checkGlyphDimensions(int glyphWidth, int requestedGlyphWidth,
  ...
  if (requestedGlyphWidth != 0) {
    ENSURE(glyphWidth == requestedGlyphWidth,
           "Expecting a packed glyph width of %d but got %d instead",
           requestedGlyphWidth, glyphWidth);
  } else {
    printf("Computed requestedGlyphWidth = %d\n", glyphWidth);
  }
  if (requestedGlyphHeight != 0) {
    ENSURE(glyphHeight == requestedGlyphHeight,
           "Expecting a packed glyph height of %d but got %d instead",
           requestedGlyphHeight, glyphHeight);
  } else {
...
void writeFontSourceFile(const char* fontSourceFilename, const char* fontName,
  ...
  fprintf(fontFile, "/* Rasterized  = %5zu bytes (%d glyphs x %d pixels)\n",
          initialDataSize, NumberOfCodePoints, glyphWidth * glyphHeight);
...
void loadCodePoint(FT_Face face, wchar_t codePoint,
  ...
  ENSURE(!FT_Load_Char(face, codePoint, FT_LOAD_RENDER),
         "Loading character 0x%08x", codePoint);
  if (combinedCharactersShift > 0 && face->glyph->bitmap_left < 0) {
    /* Combining characters are shifted left in FreeType so as to combine with
     * the previous character We don't want that, so we move it back right */
...
void drawGlyphOnBuffer(FT_GlyphSlot glyph, int glyphWidth, int glyphHeight,
  ...
  for (int y = 0; y < glyphHeight; y++) {
    for (int x = 0; x < glyphWidth; x++) {
      ...
      if (x >= glyph->bitmap_left && x < bitmap_right && y >= bitmap_top &&
          y < bitmap_bottom) {
        // We're in the buffer
        grayscalePixel =
            glyph->bitmap.buffer[(y - bitmap_top) * glyph->bitmap.pitch +
                                 (x - glyph->bitmap_left)] >>
            (8 - grayscaleBitsPerPixel);
      } else {
        grayscalePixel = 0;
  ...
  ENSURE(accumulator == 0, "Discarded accumulator data (accumulator = %d)",
         accumulator);
  ...
  ENSURE(uncompressedGlyphBufferIndex == sizeOfUncompressedGlyphBuffer,
         ...
         uncompressedGlyphBufferIndex, sizeOfUncompressedGlyphBuffer);
}
...
int compressAndAppend(uint8_t* uncompressedGlyphBuffer,
  ...
  int sizeOfCompressedGlyphBuffer = LZ4_compress_HC(
      ...
      (char*)(glyphData + glyphDataOffset), sizeOfUncompressedGlyphBuffer,
      maxGlyphDataSize - glyphDataOffset, LZ4HC_CLEVEL_MAX);
...
void generateGlyphData(FT_Face face, uint16_t* glyphDataOffset,
                       ...
                       int grayscaleBitsPerPixel) {
  int sizeOfUncompressedGlyphBuffer =
      glyphWidth * glyphHeight * grayscaleBitsPerPixel / 8;
  ENSURE(8 * sizeOfUncompressedGlyphBuffer ==
             glyphWidth * glyphHeight * grayscaleBitsPerPixel,
         "Error: the glyph size (%dx%d@%dbpp) cannot fit in an integral number "
         "of bytes",
         glyphWidth, glyphHeight, grayscaleBitsPerPixel);
  uint8_t* uncompressedGlyphBuffer =
      (uint8_t*)malloc(sizeOfUncompressedGlyphBuffer);
  int offset = 0;
  // Start with a 0
  glyphDataOffset[0] = 0;
...
void storeRenderedGlyphsImage(const char* outputImage, FT_Face face,
  ...
  bitmapImage.pixels = (pixel_t*)malloc(sizeof(pixel_t) * bitmapImage.width *
                                        bitmapImage.height);
  ENSURE(bitmapImage.pixels != NULL,
         "Allocating bitmap image of size %dx%d at %ld bytes per pixel",
         bitmapImage.width, bitmapImage.height, sizeof(pixel_t));

  drawBackground(&bitmapImage, glyphWidth, glyphHeight, grid_size,
                 maxAboveBaseline);
  drawAllGlyphsInImage(face, &bitmapImage, gridWidth, glyphWidth, glyphHeight,
                       grid_size, maxAboveBaseline);
...
void drawGlyphInImage(FT_Bitmap* glyphBitmap, image_t* image, int xOffset,
  ...
  // glyphBitmap->rows, glyphBitmap->pitch);
  ENSURE(glyphBitmap->pixel_mode == FT_PIXEL_MODE_GRAY,
...
void drawAllGlyphsInImage(FT_Face face, image_t* bitmap_image, int gridWidth,
  ...
  for (int i = 0; i < NumberOfCodePoints; i++) {
    ...
    drawGlyphInImage(&face->glyph->bitmap, bitmap_image,
                     ...
                     y * (glyphHeight + grid_size) + maxAboveBaseline -
                         face->glyph->bitmap_top);
  }
}
