...
namespace Poincare
{
  ...
  int XorExplicitNode::numberOfChildren() const { return XorExplicit::s_functionHelper.numberOfChildren(); }

  Layout XorExplicitNode::createLayout(Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return LayoutHelper::Prefix(XorExplicit(this), floatDisplayMode, numberOfSignificantDigits, XorExplicit::s_functionHelper.name());
  }

  int XorExplicitNode::serialize(char *buffer, int bufferSize, Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return SerializationHelper::Prefix(this, buffer, bufferSize, floatDisplayMode, numberOfSignificantDigits, XorExplicit::s_functionHelper.name());
  }

  Expression XorExplicitNode::shallowReduce(ReductionContext reductionContext)
  {
    return XorExplicit(this).shallowReduce(reductionContext);
  }

  template <typename T>
  Complex<T> XorExplicitNode::computeOnComplex(const std::complex<T> c, Preferences::ComplexFormat, Preferences::AngleUnit angleUnit)
  {
    if (c.imag() != 0)
    {
  ...
  Expression XorExplicit::shallowReduce(ExpressionNode::ReductionContext reductionContext)
  {
    ...
    Integer x = Integer::Xor(aq, bq, cq);
