...
CurveView::CurveView(CurveViewRange * curveViewRange, CurveViewCursor * curveViewCursor, BannerView * bannerView, CursorView * cursorView) :
  View(),
...
static float clippedFloat(float f) {
  ...
  if (f < KDCOORDINATE_MIN) {
    ...
    /* This will return NAN if f is NAN */
    return f;
  }
}
...
void CurveView::computeLabels(Axis axis) {
  ...
  for (int i = 0; i < axisLabelsCount; i++) {
    ...
    if (axis == Axis::Horizontal) {
      ...
      labelMaxGlyphLength = std::min<int>(labelMaxGlyphLengthSize(), pixelsPerLabel/KDFont::Font(k_font)->glyphSize().width());
    }
...
void CurveView::drawLabelsAndGraduations(KDContext * ctx, KDRect rect, Axis axis, bool shiftOrigin, bool graduationOnly, bool fixCoordinate, KDCoordinate fixedCoordinate, KDColor backgroundColor) const {
  ...
  KDCoordinate maximalVerticalPosition = graduationOnly ? viewHeight : viewHeight - KDFont::Font(k_font)->glyphSize().height() - k_labelMargin;
  ...
  if (axis == Axis::Horizontal) {
    ...
    KDCoordinate minimalHorizontalPosition = graduationOnly ? 0 : k_labelMargin + KDFont::Font(k_font)->glyphSize().width() * 3; // We want do display at least 3 characters left of the Y axis
  ...
  if (axis == Axis::Vertical) {
     ...
     * labels */
    int horizontalLabelsMargin = KDFont::Font(k_font)->glyphSize().height() * 2;
  ...
  for (int i = minDrawnLabel; i < maxDrawnLabel; i++) {
    ...
    if (shouldDrawLabelAtPosition(labelValue)) {
      ...
      KDSize textSize = KDFont::Font(k_font)->stringSize(labelI);
      ...
      if (axis == Axis::Horizontal) {
        ...
        if (floatingLabels == FloatingPosition::Min) {
          ...
          position = KDPoint(position.x(), viewHeight - KDFont::Font(k_font)->glyphSize().height() - k_labelMargin);
        }
      } else {
...
void CurveView::drawHistogram(KDContext * ctx, KDRect rect, EvaluateYForX yEvaluation, void * model, void * context, float firstBarAbscissa, float barWidth, bool fillBar, KDColor defaultColor, KDColor highlightColor, KDCoordinate borderWidth, KDColor borderColor,  float highlightLowerBound, float highlightUpperBound) const {
  ...
  for (float x = rectMinLowerBound; x < rectMaxUpperBound; x += step) {
    ...
    KDCoordinate pxf = std::round(floatToPixel(Axis::Horizontal, x)) + borderWidth;
...
static bool shouldDashedCurveStamp(int stampNumber) {
...
void CurveView::layoutSubviews(bool force) {
  ...
  if (m_bannerView != nullptr) {
    ...
    if (!(m_bannerView->bounds() == oldFrame)) {
      const KDCoordinate dirtyHeight = std::max(oldFrame.height(), m_bannerView->bounds().height()) + KDFont::Font(k_font)->glyphSize().height() + 2 * k_labelMargin;
