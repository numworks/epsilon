...
size_t CursorIndexInCommand(const char* text, const char* stoppingPosition,
                            bool ignoreCodePointSytem) {
  assert(stoppingPosition == nullptr || text <= stoppingPosition);
  UTF8Decoder decoder(text);
  const char* currentPointer = text;
  int numberOfIgnoredChar = 0;
  CodePoint codePoint = decoder.nextCodePoint();
  while ((stoppingPosition == nullptr || currentPointer < stoppingPosition) &&
         codePoint != UCodePointNull) {
