...
void BoxPlotPolicy::drawBar(const AbstractPlotView * plotView, KDContext * ctx, KDRect rect, float calculation, float lowBound, float upBound, KDColor color, bool isSelected) const {
  ...
  if (isSelected) {
    lowBound = plotView->pixelToFloat(Axis::Vertical, k_verticalSideSize + BoxHeight(m_store->numberOfValidSeries(Shared::DoublePairStore::DefaultValidSeries)) + k_chevronMargin - 1);
