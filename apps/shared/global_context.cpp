...
const Expression GlobalContext::protectedExpressionForSymbolAbstract(const Poincare::SymbolAbstract & symbol, bool clone, Poincare::ContextWithParent * lastDescendantContext) {
  Ion::Storage::Record r = SymbolAbstractRecordWithBaseName(symbol.name());
  return expressionForSymbolAndRecord(symbol, r, lastDescendantContext ? static_cast<Context *>(lastDescendantContext) : static_cast<Context *>(this));
}
...
const Expression GlobalContext::expressionForSequence(const SymbolAbstract & symbol, Ion::Storage::Record r, Context * ctx) {
  ...
  PoincareHelpers::CloneAndSimplify(&rank, ctx, ReductionTarget::SystemForApproximation);
...
Ion::Storage::Record::ErrorStatus GlobalContext::setExpressionForActualSymbol(const Expression & expression, const SymbolAbstract & symbol, Ion::Storage::Record previousRecord) {
  ...
  PoincareHelpers::CloneAndSimplify(&expressionToStore, this, ReductionTarget::User, SymbolicComputation::ReplaceAllSymbolsWithDefinitionsOrUndefined);
