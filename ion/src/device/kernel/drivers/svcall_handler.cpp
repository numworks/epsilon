...
void __attribute__((externally_visible)) svcall_handler(uint32_t processStackPointer, uint32_t exceptReturn, uint32_t svcNumber) {
  ...
  constexpr unsigned authentificationRequired[] = {SVC_USB_WILL_EXECUTE_DFU, SVC_USB_DID_EXECUTE_DFU, SVC_BOARD_SWITCH_EXECUTABLE_SLOT, SVC_BOARD_DOWNGRADE_TRUST_LEVEL, SVC_FLASH_MASS_ERASE, SVC_FLASH_ERASE_SECTOR, SVC_FLASH_WRITE_MEMORY, SVC_LED_SET_COLOR, SVC_LED_UPDATE_COLOR_WITH_PLUG_AND_CHARGE, SVC_LED_SET_BLINKING, SVC_PERSISTING_BYTES_READ, SVC_PERSISTING_BYTES_WRITE};
