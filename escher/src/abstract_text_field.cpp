...
bool AbstractTextField::privateHandleSelectEvent(Ion::Events::Event event) {
  ...
  if (event == Ion::Events::ShiftLeft || event == Ion::Events::ShiftRight || event == Ion::Events::ShiftUp || event == Ion::Events::ShiftDown) {
    selectLeftRight((event == Ion::Events::ShiftLeft || event == Ion::Events::ShiftUp) ? OMG::NewDirection::Left() : OMG::NewDirection::Right(), event == Ion::Events::ShiftUp || event == Ion::Events::ShiftDown);
    return true;
  }
  return false;
}
