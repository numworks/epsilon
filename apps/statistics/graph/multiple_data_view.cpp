...
KDCoordinate MultipleDataView::subviewHeight() {
  int numberDataSubviews = m_store->numberOfActiveSeries(
...
int MultipleDataView::numberOfSubviews() const {
  int result = m_store->numberOfActiveSeries(
      Shared::DoublePairStore::DefaultActiveSeriesTest);
...
View* MultipleDataView::subviewAtIndex(int index) {
  ...
  return plotViewForSeries(m_store->seriesIndexFromActiveSeriesIndex(
      index, Shared::DoublePairStore::DefaultActiveSeriesTest));
}
