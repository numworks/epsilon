...
namespace Poincare
{
  ...
  int AndExplicitNode::numberOfChildren() const { return AndExplicit::s_functionHelper.numberOfChildren(); }

  Layout AndExplicitNode::createLayout(Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return LayoutHelper::Prefix(AndExplicit(this), floatDisplayMode, numberOfSignificantDigits, AndExplicit::s_functionHelper.name());
  }

  int AndExplicitNode::serialize(char *buffer, int bufferSize, Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return SerializationHelper::Prefix(this, buffer, bufferSize, floatDisplayMode, numberOfSignificantDigits, AndExplicit::s_functionHelper.name());
  }

  Expression AndExplicitNode::shallowReduce(ReductionContext reductionContext)
  {
  ...
  Expression AndExplicit::shallowReduce(ExpressionNode::ReductionContext reductionContext)
  {
    ...
    Integer x = Integer::And(aq, bq, cq);
