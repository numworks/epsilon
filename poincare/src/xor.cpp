...
namespace Poincare
{
  ...
  int XorNode::numberOfChildren() const { return Xor::s_functionHelper.numberOfChildren(); }

  Layout XorNode::createLayout(Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return LayoutHelper::Prefix(Xor(this), floatDisplayMode, numberOfSignificantDigits, Xor::s_functionHelper.name());
  }

  int XorNode::serialize(char *buffer, int bufferSize, Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
  ...
  Expression XorNode::shallowReduce(ReductionContext reductionContext)
  {
    return Xor(this).shallowReduce(reductionContext);
  }

  template <typename T>
  Complex<T> XorNode::computeOnComplex(const std::complex<T> c, Preferences::ComplexFormat, Preferences::AngleUnit angleUnit)
  {
    if (c.imag() != 0)
    {
