...
def get_dfu_devices_with_vid_pids(vid_pids):
    devices = [
        get_dfu_devices(idVendor=vid_pid["VID"], idProduct=vid_pid["PID"])
...
def exit_dfu(address=0x08000000):
    ...
    __dev.ctrl_transfer(0x21, __DFU_DNLOAD, 2, __DFU_INTERFACE, None, __TIMEOUT)
...
def main():
    ...
    if args.path:
        ...
        if args.path.endswith(".dfu"):
            ...
            if not address:
                print(
                    "Missing address: for raw binary download, use the --dfuse-address option"
                )
                return
