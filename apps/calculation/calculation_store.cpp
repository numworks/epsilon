...
ExpiringPointer<Calculation> CalculationStore::push(const char * text, Poincare::Context * context, HeightComputer heightComputer) {
  ...
  if (CircuitBreakerRun(checkpoint)) {
    ...
    Expression inputExpression = Expression::Parse(text, context).replaceSymbolWithExpression(Symbol::Ans(), ans);
    cursor = pushSerializedExpression(cursor, inputExpression, maxNumberOfDigits);
    ...
    PoincareHelpers::ParseAndSimplifyAndApproximate(inputText, &inputExpression, &exactOutputExpression, &approximateOutputExpression, context);
    ...
    if (exactOutputExpression.type() == ExpressionNode::Type::Store) {
      ...
      if (storeExpression.childAtIndex(1).type() == ExpressionNode::Type::Symbol &&
          ExpressionDisplayPermissions::ShouldOnlyDisplayApproximation(inputExpression, exactStoredExpression, context))
      {
        storeExpression.replaceChildAtIndexInPlace(0, approximateStoredExpression);
      }
      storeExpression.storeValueForSymbol(context);
      exactOutputExpression = context->expressionForSymbolAbstract(storeExpression.symbol(), false);
    ...
    if (ExamModeConfiguration::unitsAreForbidden() && approximateOutputExpression.hasUnit()) {
    ...
    Calculation * newCalculation = reinterpret_cast<Calculation *>(endOfCalculations());
    SetCalculationHeights(newCalculation, heightComputer, context);

    /* Now that the calculation is fully built, we can finally update
     * m_numberOfCalculations. As that is the only variable tracking the state of
     * the store, updating it only at the end of the push ensures that, should an
     * interruption occur, all the temporary states are silently discarded and no
     * ill-formed Calculation is stored. */
...
void CalculationStore::recomputeHeightsIfPreferencesHaveChanged(Poincare::Preferences * preferences, HeightComputer heightComputer) {
...
size_t CalculationStore::privateDeleteCalculationAtIndex(int index, char * shiftedMemoryEnd) {
  char * deletionStart = index == numberOfCalculations() - 1 ? m_buffer : endOfCalculationAtIndex(index + 1);
...
ExpiringPointer<Calculation> CalculationStore::errorPushUndefined(HeightComputer heightComputer) {
...
char * CalculationStore::pushSerializedExpression(char * location, Expression e, int numberOfSignificantDigits) {
  while (true) {
    int availableSize = spaceForNewCalculations(location);
    int length = availableSize > 0 ? PoincareHelpers::Serialize(e, location, availableSize, numberOfSignificantDigits) : 0;
...
char * CalculationStore::pushUndefined(char * location) {
  return pushSerializedExpression(location, Undefined::Builder(), m_inUsePreferences.numberOfSignificantDigits());
}
