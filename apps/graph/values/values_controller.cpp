...
int ValuesController::columnToFreeze() {
  ...
  for (size_t symbolType = 0; symbolType < k_maxNumberOfSymbolTypes; symbolType++) {
    ...
    if (cumulatedWidthBeforeIndex(indexOfLastColumn) >= offset().x()) {
      /* We take the first candidate for which last column is not hidden by scroll.
       * Indeed, if last column is hidden (even partly) by scroll, it means focus
       * is not on this subtable so we don't want to freeze this abscissa. */
...
void ValuesController::createMemoizedLayout(int column, int row, int index) {
  ...
  if (isDerivative) {
    ...
    if (simplificationFailure || !m_exactValuesAreActivated || ExpressionDisplayPermissions::ShouldOnlyDisplayApproximation(function->originalEquation(), result, context)) {
