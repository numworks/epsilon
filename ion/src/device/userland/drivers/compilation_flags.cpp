...
/*
 * We use an uint32_t as a bit array representing the compilation flags. The
 * most significant 16 bits concern the kernel compilation and the least
 * significant 16 bits concern the userland.
 *
...
uint16_t userlandCompilationFlags() {
  ...
  assert(Device::Board::securityLevel() <
         0xFF);  // Should the SECURITY_LEVEL exceed 0xFF, , we'll find another
                 // way to represent it
  uint8_t securityLevel = 0xFF & Device::Board::securityLevel();

  return DEBUG | (ASSERTIONS << 1) | (ExternalApps::allowThirdParty() << 2) |
         (Device::Board::isRunningSlotA() << 3) | (externalAppsAPILevel << 4) |
         (securityLevel << 8);
}
...
const char* compilationFlags() {
  static char compilationFlagsBuffer[OMG::Print::MaxLengthOfUInt32(
                                     ...
                                     1] = {0};
  uint32_t flags =
      userlandCompilationFlags() | (kernelCompilationFlags() << 16);
...
const char* runningBootloader() {
  static char crcBuffer[OMG::Print::MaxLengthOfUInt32(OMG::Base::Hexadecimal) +
                        1] = {0};
  size_t length = OMG::Print::UInt32(OMG::Base::Hexadecimal, bootloaderCRC32(),
