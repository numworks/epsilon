...
bool Script::NameCompliant(const char* name) {
  ...
  while (c != UCodePointNull) {
    if (c == '.' &&
        strcmp(decoder.stringPosition(), ScriptStore::k_scriptExtension) == 0) {
      return true;
    }
...
Script::ErrorStatus Script::Create(const char* name, const char* content) {
  ...
  ErrorStatus err = Ion::Storage::FileSystem::sharedFileSystem
                        ->createRecordWithFullNameAndDataChunks(
                            name, dataChunks, sizeChunks, 2);
