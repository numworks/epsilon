...
QUIZ_CASE(pcj_simplification_rational_power) {
  ...
  // Integer power overflows
  simplifies_to("1.0089^77.5", "(3×(10089/10000)^77×√(1121))/100");
  simplifies_to("1.00089^77.5", "(3×(100089/100000)^77×√(111210))/1000");
  simplifies_to("100000^100000×100001^100001", "100001×10000100000^100000");
  simplifies_to("100000^100000/100001^100001", "(100000/100001)^100000/100001");
  simplifies_to("100000^(-100000)×100001^(-100001)",
                "(1/10000100000)^100000/100001");
}
...
QUIZ_CASE(pcj_tree_simplification) {
  simplifies_to(KACos(23_fe), KMult(3.8281684_fe, i_e),
                {.m_complexFormat = ComplexFormat::Cartesian});
  simplifies_to(KACos(KMult(3.5_fe, i_e)),
                KAdd(1.570796251297_fe, KMult(-1.9657205_fe, i_e)),
                {.m_complexFormat = ComplexFormat::Cartesian});
