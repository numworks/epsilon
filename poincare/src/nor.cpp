...
namespace Poincare
{
  ...
  int NorNode::numberOfChildren() const { return Nor::s_functionHelper.numberOfChildren(); }

  Layout NorNode::createLayout(Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return LayoutHelper::Prefix(Nor(this), floatDisplayMode, numberOfSignificantDigits, Nor::s_functionHelper.name());
  }

  int NorNode::serialize(char *buffer, int bufferSize, Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return SerializationHelper::Prefix(this, buffer, bufferSize, floatDisplayMode, numberOfSignificantDigits, Nor::s_functionHelper.name());
  }

  Expression NorNode::shallowReduce(ReductionContext reductionContext)
  {
    return Nor(this).shallowReduce(reductionContext);
  }

  template <typename T>
  Complex<T> NorNode::computeOnComplex(const std::complex<T> c, Preferences::ComplexFormat, Preferences::AngleUnit angleUnit)
  {
    if (c.imag() != 0)
    {
  ...
  Expression Nor::shallowReduce(ExpressionNode::ReductionContext reductionContext)
  {
    ...
    Integer x = Integer::LogicalNor(aq, bq);
