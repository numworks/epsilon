...
void UnitListController::computeAdditionalResults(
  ...
  Layout exactOutputLayout =
      Shared::PoincareHelpers::CreateLayout(exactOutput, symbolContext)
          .cloneAndTurnEToTenPowerLayout(false);
  for (size_t i = 0; i < k_maxNumberOfExpressionCells; i++) {
    if (!expressions[i].isUninitialized() && !expressions[i].isUndefined()) {
      Layout layout =
          Shared::PoincareHelpers::CreateLayout(expressions[i], symbolContext)
              .cloneAndTurnEToTenPowerLayout(false);
  ...
  // Compute SI Value
  m_SIValue = approximatedSIExpression.approximateUnitToRealScalar<double>(
      angleUnit(), complexFormat(), EmptySymbolContext{});
  // Set upper and lower reference values
  m_numberOfBufferCells = UnitComparison::FindUpperAndLowerReferenceValues(
      m_SIValue, approximatedSIExpression, symbolContext, m_referenceValues,
