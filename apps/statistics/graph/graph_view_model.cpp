...
I18n::Message GraphViewModel::MessageForGraphView(GraphView graphView) {
  ...
  I18n::Message::CumulativeFrequency,
  I18n::Message::NormalProbabilityPlot
  };
  return k_messages[IndexOfGraphView(graphView)];
}

const Escher::Image * const GraphViewModel::ImageForGraphView(GraphView graphView) {
  const Escher::Image * const k_images[k_numberOfGraphViews] = {
    ImageStore::HistogramIcon,
    ImageStore::BoxIcon,
    ImageStore::CumulativeIcon,
    ImageStore::StatisticsNormalIcon
  };
...
uint8_t GraphViewModel::IndexOfGraphView(GraphView graphView) {
  assert(static_cast<uint8_t>(graphView) >= 0 && static_cast<uint8_t>(graphView) < k_numberOfGraphViews);
