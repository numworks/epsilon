...
enum class AssetFormat { JPG, PNG, None };

png_size_t readSize = 0;

void ReadDataFromMemory(png_structp png, png_bytep outBytes,
                        png_size_t byteSize) {
...
bool readPNG(unsigned char *start, size_t size, unsigned char **bitmapData,
             unsigned int *width, unsigned int *height,
             unsigned int *bytesPerPixel, Uint32 *pixelFormat) {
  readSize = 0;
  png_structp png =
...
bool readJPG(const unsigned char *start, size_t size,
             unsigned char **bitmapData, unsigned int *width,
  ...
  *bitmapData = new unsigned char[*height * *width * *bytesPerPixel];
  ...
  while (info.output_scanline < info.output_height) {
    ...
    buffer_array[0] =
        *bitmapData + info.output_scanline * *width * *bytesPerPixel;
...
SDL_Texture *loadImage(SDL_Renderer *renderer, const char *identifier) {
  ...
  for (size_t i = 0; i < std::size(resources_addresses); i++) {
    if (strcmp(identifier, resources_addresses[i].identifier()) == 0) {
      if (strcmp(jpgExtension,
        ...
        assert(strcmp(pngExtension, identifier + strlen(identifier) -
                                        strlen(pngExtension)) == 0);
        (void)pngExtension;  // Silent warning
        format = AssetFormat::PNG;
      }
      assetStart = resources_addresses[i].start();
      assertSize =
          resources_addresses[i].end() - resources_addresses[i].start();
      break;
    }
  }
  ...
  switch (format) {
    case AssetFormat::JPG:
      if (!readJPG(assetStart, assertSize, &bitmapData, &width, &height,
    ...
    default:
      ...
      if (!readPNG(assetStart, assertSize, &bitmapData, &width, &height,
                   &bytesPerPixel, &pixelFormat)) {
        return nullptr;
      }
  }
  ...
  SDL_Texture *texture = SDL_CreateTexture(
      renderer, pixelFormat, SDL_TEXTUREACCESS_STATIC, width, height);

  SDL_UpdateTexture(texture, nullptr, bitmapData, width * bytesPerPixel);
...
class RGB888Pixel {
 public:
  ...
  RGB888Pixel(KDColor c)
      : m_red(c.red()), m_green(c.green()), m_blue(c.blue()) {}
