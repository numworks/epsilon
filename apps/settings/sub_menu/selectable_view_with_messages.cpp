...
SelectableViewWithMessages::SelectableViewWithMessages(
    ...
    : m_selectableTableView(selectableTableView), m_numberOfMessages(0) {
  for (int i = 0; i < k_maxNumberOfLines; i++) {
    m_messageLines[i].setFont(KDFont::Size::Small);
    m_messageLines[i].setAlignment(KDContext::k_alignCenter,
...
void SelectableViewWithMessages::setMessages(I18n::Message* m,
...
void SelectableViewWithMessages::layoutSubviews(bool force) {
  ...
  KDCoordinate defOrigin = std::max<KDCoordinate>(
      bounds().height() -
          (m_numberOfMessages ? Metric::CommonBottomMargin : 0) -
          m_numberOfMessages * textHeight,
      Metric::CommonBottomMargin + k_minSelectableTableViewHeight +
