...
Expression Derivative::shallowReduce(const ExpressionNode::ReductionContext& reductionContext) {
  ...
  if (reducedDerivative.type() == ExpressionNode::Type::Dependency) {
    ...
    reducedDerivative.replaceWithInPlace(reducedDerivative.childAtIndex(0));
    reducedDerivative = childAtIndex(0);
  }
  Dependency d = Dependency::Builder(reducedDerivative, l);
