...
bool IsInRegion(const ContinuousFunction* f, float x, float y) {
  ...
  switch (area) {
    case ContinuousFunctionProperties::AreaType::Above:
      // Relying on tOut > NAN being false here
      return tOut > fOut;
    case ContinuousFunctionProperties::AreaType::Below:
      // Relying on tOut < NAN being false here
      return tOut < fOut;
    default: {
      if (std::isnan(fOut) || tOut >= fOut) {
...
void GraphView::drawTangent(KDContext* ctx, KDRect rect,
  ...
  assert(context());
  float tangentParameterA = f->approximateSlope(m_cursor->t());
...
void GraphView::drawPointsOfInterest(KDContext* ctx, KDRect rect) {
  ...
  if (!m_intersectionRegion) {
    drawPointsOfInterest(ctx, rect, selectedRecord(), shouldComputePoints);
  } else {
...
void GraphView::drawPointsOfInterest(KDContext* ctx, KDRect rect,
                                     Ion::Storage::Record selectedRec,
