...
class SpacerCell : public HighlightCell {
 public:
  ...
  KDSize minimalSizeForOptimalDisplay() const override {
    return KDSize(0, k_defaultHeight);
  }
  void drawRect(KDContext* ctx, KDRect rect) const override {
    /* Drawing rects overlap in lists and spacer only redraws its own part. */
    ctx->fillRect(KDRect(rect.left(), rect.top() + k_overlap, rect.width(),
                         rect.height() - k_overlap),
                  Palette::WallScreen);
  }
 ...
 private:
  ...
  constexpr static KDCoordinate k_overlap =
      Escher::Metric::CellSeparatorThickness;
  ;
  constexpr static KDCoordinate k_defaultHeight =
      Escher::Metric::CommonMenuMargin + k_overlap;
};
