...
class OptionalAxes {
 protected:
  void drawAxesAndGrid(const Shared::AbstractPlotView* plotView, KDContext* ctx,
                       KDRect rect) const;
  void reloadAxes(Shared::AbstractPlotView* plotView) {
    m_axes.reloadAxes(plotView);
  }
 ...
 private:
  Shared::PlotPolicy::Axes<Shared::PlotPolicy::WithGrid,
                           ...
                           Shared::PlotPolicy::NoAxis>
      m_grid;
  Shared::PlotPolicy::Axes<Shared::PlotPolicy::NoGrid,
                           Shared::PlotPolicy::HorizontalLabeledAxis,
                           Shared::PlotPolicy::VerticalLabeledAxis>
      m_axes;
};

class PyplotPolicy {
 public:
  void setMicroPythonExecutionEnvironment(
      MicroPython::ExecutionEnvironment* env) {
    m_micropythonEnvironment = env;
  }

 protected:
  void drawPlot(const Shared::AbstractPlotView* plotView, KDContext* ctx,
                KDRect rect) const;
  void traceDot(const Shared::AbstractPlotView* plotView, KDContext* ctx,
                KDRect r, PlotStore::Dot dot) const;
  void traceSegment(const Shared::AbstractPlotView* plotView, KDContext* ctx,
                    KDRect r, PlotStore::Segment segment) const;
  void traceRect(const Shared::AbstractPlotView* plotView, KDContext* ctx,
                 KDRect r, PlotStore::Rect rect) const;
  void traceLabel(const Shared::AbstractPlotView* plotView, KDContext* ctx,
...
class PyplotView : public Shared::PlotView<OptionalAxes, PyplotPolicy,
