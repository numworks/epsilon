...
parser.add_argument('--image', type=str, action='append', help='an image file to convert into a Base64 data URL')
...
for line in open(args.file).readlines():
  ...
  if '<img ' in line:
    for filename, data in images.items():
      line = line.replace(f'src="{filename}"', f'src="data:image/{filename.split(".")[-1]};base64,{data}"')
  if '<source ' in line:
    for filename, data in images.items():
      line = line.replace(f'srcset="{filename}"', f'srcset="data:image/{filename.split(".")[-1]};base64,{data}"')
  print(line, end = '')
