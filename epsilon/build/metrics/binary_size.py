...
def iso_separate(string):
    ...
    return (
        string.replace("_", space).replace("+", "+" + space).replace("-", "-" + space)
    )
...
def format_html_section_table(
    grouped_sections, grouped_files, file_section_sizes, show_file_detail
):
    output = ""
    output += "<table>"
    ...
    if show_section_groups:
        ...
        output += "<tr>"
        output += (
            f'<td rowspan="2" colspan="{show_section_groups + show_file_detail}"></td>'
        )
        for title, sections in grouped_sections:
            output += (
                f'<th colspan="{len(sections)}" align="left">{title} in bytes</th>'
            )
        output += "</tr>"
...
parser.add_argument(
    ...
    help="Show a summarized version",
)
parser.add_argument(
    "--app-sizes", action=argparse.BooleanOptionalAction, help="Show app sizes"
)
...
if args.app_sizes:
    ...
    print(
        format_html_section_table(
            grouped_sections, grouped_files, file_section_sizes, args.summarize
        )
    )
