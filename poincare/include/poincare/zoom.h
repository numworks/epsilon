...
public:
  ...
  Zoom(float xMin, float xMax, float normalYXRatio) : m_xMax(xMax), m_xMin(xMin), m_normalRatio(normalYXRatio), m_sampleUpToDate(false) {}

  Range2D range() const { return m_range; }
  void setFunction(Solver<float>::FunctionEvaluation f, const void * model);
...
private:
  ...
  float approximate(float x) const { assert(m_function); return m_function(x, m_model); }
  void sample();
  ...
  float m_xMax, m_xMin;
  float m_normalRatio;
  Solver<float>::FunctionEvaluation m_function;
  const void * m_model;
