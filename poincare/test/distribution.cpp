...
struct DistributionTestCase {
  DistributionTestCase(double K, double X, double d, double p, float precision = 1e-7) :
...
DistributionTestCase getStudentTestCase(int i) {
  static DistributionTestCase s_studentTests[k_numberOfStudentTestCases] {
    ...
    DistributionTestCase(2.57, -10, 0.0005030925333730742, 0.001996706792893251),
    DistributionTestCase(3, -10, 0.0003118082168470876, 0.0010641995292070756),
    DistributionTestCase(4, -10, 0.00010879223472902223, 0.00028100181135799556),
    DistributionTestCase(5, -10, 4.098981641534331e-05, 8.547378787148179e-05),
    DistributionTestCase(6, -10, 1.6514084372279296e-05, 2.8959913774768124e-05),
    DistributionTestCase(7, -10, 7.051932309805197e-06, 1.069710144538641e-05),
    DistributionTestCase(10, -10, 7.284685722912541e-07, 7.947765877982051e-07),
    DistributionTestCase(20, -10, 2.6600849800550903e-09, 1.5818908793571965e-09),
    DistributionTestCase(50, -10, 2.7048721527008514e-13, 8.038667344167695e-14, 1e-3),
    DistributionTestCase(0.28, -1.0, 0.08462111434249496, 0.34697350341129507),
    DistributionTestCase(1, -1.0, 0.15915494309189537, 0.24999999999999978),
    DistributionTestCase(2.57, -1.0, 0.20174670047005813, 0.20098201301585422),
    ...
    DistributionTestCase(0.28, -0.2, 0.20549304829293147, 0.45651086361470994, 1e-5),
    DistributionTestCase(1, -0.2, 0.3060671982536449, 0.43716704181099886, 1e-5),
    ...
    DistributionTestCase(20, -0.2, 0.38580918607411935, 0.42175008348394183, 1e-5),
    DistributionTestCase(50, -0.2, 0.38894005621150207, 0.4211459041217918),
    DistributionTestCase(500, -0.2, 0.39083190132875784, 0.42078094851278847, 1e-5),
    ...
    DistributionTestCase(500, 0.3, 0.3811644097585056, 0.6178490820245964, 1e-5),
    ...
    DistributionTestCase(5, 18, 1.3324661240937334e-06, 0.9999951397795279, 1e-5),
    DistributionTestCase(6, 18, 3.1018888949841117e-07, 0.9999990543824999, 1e-5),
    DistributionTestCase(7, 18, 7.700718899661956e-08, 0.999999798186122, 1e-3),
  };
...
QUIZ_CASE(poincare_student_distribution) {
  for (int i = 0; i < k_numberOfStudentTestCases; i++) {
    ...
    assert_roughly_equal(StudentDistribution::EvaluateAtAbscissa(t.x, t.k),
                       t.density,
                       t.precision);
    assert_roughly_equal(StudentDistribution::CumulativeDistributiveFunctionAtAbscissa(t.x, t.k),
                       t.probability,
                       t.precision);
    assert_roughly_equal(
        StudentDistribution::CumulativeDistributiveInverseForProbability(t.probability, t.k),
        t.x,
        t.precision / 10);

    // floats
    assert_roughly_equal<float>(StudentDistribution::EvaluateAtAbscissa<float>(t.x, t.k),
                              t.density,
                              std::sqrt(t.precision));
    assert_roughly_equal<float>(
        StudentDistribution::CumulativeDistributiveFunctionAtAbscissa<float>(t.x, t.k),
        t.probability,
        std::sqrt(t.precision));
    assert_roughly_equal<float>(
        StudentDistribution::CumulativeDistributiveInverseForProbability<float>(t.probability,
                                                                                    t.k),
        t.x,
        std::sqrt(t.precision));
  }
  ...
  // Special cases p=0
  // double
  assert_roughly_equal<double>(StudentDistribution::EvaluateAtAbscissa<double>(INFINITY, 5.), 0);
  assert_roughly_equal<double>(StudentDistribution::EvaluateAtAbscissa<double>(-INFINITY, 5.), 0);
  assert_roughly_equal<double>(StudentDistribution::CumulativeDistributiveFunctionAtAbscissa<double>(INFINITY, 3.),
                     1.);
  assert_roughly_equal<double>(StudentDistribution::CumulativeDistributiveFunctionAtAbscissa<double>(-INFINITY, 3.),
                     0.);
  assert_roughly_equal<double>(StudentDistribution::CumulativeDistributiveInverseForProbability<double>(0, 5.),
                     -INFINITY);
  assert_roughly_equal<double>(StudentDistribution::CumulativeDistributiveInverseForProbability<double>(1, 5.),
                     INFINITY);

  // floats
  assert_roughly_equal<float>(StudentDistribution::EvaluateAtAbscissa<float>(INFINITY, 5.), 0);
  assert_roughly_equal<float>(StudentDistribution::EvaluateAtAbscissa<float>(-INFINITY, 5.), 0);
  assert_roughly_equal<float>(StudentDistribution::CumulativeDistributiveFunctionAtAbscissa<float>(INFINITY, 3.),
                     1.);
  assert_roughly_equal<float>(StudentDistribution::CumulativeDistributiveFunctionAtAbscissa<float>(-INFINITY, 3.),
                     0.);
  assert_roughly_equal<float>(StudentDistribution::CumulativeDistributiveInverseForProbability<float>(0, 5.),
                     -INFINITY);
  assert_roughly_equal<float>(StudentDistribution::CumulativeDistributiveInverseForProbability<float>(1, 5.),
                     INFINITY);
}
...
DistributionTestCase getChi2TestCase(int i) {
  static DistributionTestCase s_chi2Tests[k_numberOfChi2TestCases] {
    ...
    DistributionTestCase(0.28, 0.25, 0.39445708636423615, 0.7863197131812323, 1e-6),
    DistributionTestCase(0.28, 0.7, 0.1299354327320162, 0.8857276344687262, 1e-6),
    DistributionTestCase(0.28, 2, 0.02750012716139315, 0.9650437489682855, 1e-6),
    DistributionTestCase(0.28, 10, 0.000126195122829492, 0.999780321882247, 1e-6),
    ...
    DistributionTestCase(2.57, 0.123, 0.2360000701221212, 0.023209437129391372, 1e-6),
    DistributionTestCase(2.57, 0.25, 0.27109679181024543, 0.055740896473783726, 1e-6),
    DistributionTestCase(2.57, 0.7, 0.29030075052193394, 0.1851697163773158, 1e-6),
    ...
    DistributionTestCase(3, 10, 0.008500366602520343, 0.9814338645369568, 1e-6),
    DistributionTestCase(5, 0, 0.0, 0.0),
    DistributionTestCase(5, 0.123, 0.005394328218580669, 0.0002701288757651666, 1e-6),
    DistributionTestCase(5, 0.25, 0.014669388615179144, 0.001520818553368441, 1e-6),
    DistributionTestCase(5, 0.7, 0.054882363062039155, 0.01703132481196757, 1e-6),
    DistributionTestCase(5, 2, 0.1383691658068649, 0.15085496391539038, 1e-6),
    DistributionTestCase(5, 10, 0.028334555341734475, 0.9247647538534878, 1e-6),
    DistributionTestCase(5, 30.85618994044592, 4.5442869596536075e-06, .99999, 1e-6),
    DistributionTestCase(10, 0, 0.0, 0.0),
    DistributionTestCase(10, 0.25, 4.488611361615981e-06, 2.2919102136524132e-07),
    DistributionTestCase(10, 0.7, 0.00022030678429878006, 3.273617475313252e-05, 1e-6),
    DistributionTestCase(10, 2, 0.00766415502440505, 0.003659846827343713, 1e-6),
    DistributionTestCase(20, 0, 0.0, 0.0),
    DistributionTestCase(20, 5.3, 0.0006273926980786215, 0.0004347478563383611, 1e-6),
    DistributionTestCase(20, 20, 0.06255501786056658, 0.5420702855281478, 1e-5),
    DistributionTestCase(100, 0, 0.0, 0.0),
    DistributionTestCase(100, 100, 0.028162503162596778, 0.5188083154720433, 1e-6),
  };
...
QUIZ_CASE(probability_chi2_law) {
  for (unsigned int i = 0; i < k_numberOfChi2TestCases; i++) {
    ...
    // double
    assert_roughly_equal(Chi2Distribution::EvaluateAtAbscissa(t.x, t.k), t.density, t.precision);
    assert_roughly_equal(Chi2Distribution::CumulativeDistributiveFunctionAtAbscissa(t.x, t.k),
                       t.probability,
                       t.precision);
    assert_roughly_equal(
        Chi2Distribution::CumulativeDistributiveInverseForProbability(t.probability, t.k),
        t.x,
        t.precision / 10);

    // floats
    assert_roughly_equal<float>(Chi2Distribution::EvaluateAtAbscissa<float>(t.x, t.k),
                              t.density,
                              sqrt(t.precision));
    assert_roughly_equal<float>(
        Chi2Distribution::CumulativeDistributiveFunctionAtAbscissa<float>(t.x, t.k),
        t.probability,
        sqrt(t.precision));
    assert_roughly_equal<float>(
        Chi2Distribution::CumulativeDistributiveInverseForProbability<float>(t.probability,
                                                                                 t.k),
        t.x,
        sqrt(t.precision));
  }
  ...
  assert_roughly_equal<double>(Chi2Distribution::EvaluateAtAbscissa<double>(INFINITY, 5.), 0);
  assert_roughly_equal<double>(Chi2Distribution::EvaluateAtAbscissa<double>(0, 2.), 0);
  assert_roughly_equal<double>(Chi2Distribution::CumulativeDistributiveFunctionAtAbscissa<double>(INFINITY, 3.),
                     1.);
  assert_roughly_equal<double>(Chi2Distribution::CumulativeDistributiveFunctionAtAbscissa<double>(0, 3.), 0.);
  assert_roughly_equal<double>(Chi2Distribution::CumulativeDistributiveFunctionAtAbscissa<double>(0, .3), 0.);
  assert_roughly_equal<double>(Chi2Distribution::CumulativeDistributiveInverseForProbability<double>(0, 5.), 0.);
  assert_roughly_equal<double>(Chi2Distribution::CumulativeDistributiveInverseForProbability<double>(1, 5.),
                     INFINITY);

  // floats
  quiz_assert(std::isnan(Chi2Distribution::EvaluateAtAbscissa<float>(-1, 5.)));
  assert_roughly_equal<float>(Chi2Distribution::EvaluateAtAbscissa<float>(INFINITY, 5.f), 0);
  assert_roughly_equal<float>(Chi2Distribution::EvaluateAtAbscissa<float>(0, 2.f), 0);
  assert_roughly_equal<float>(Chi2Distribution::CumulativeDistributiveFunctionAtAbscissa<float>(INFINITY, 3.f),
                     1.f);
  assert_roughly_equal<float>(Chi2Distribution::CumulativeDistributiveFunctionAtAbscissa<float>(0, 3.f), 0.f);
  assert_roughly_equal<float>(Chi2Distribution::CumulativeDistributiveFunctionAtAbscissa<float>(0, .3f), 0.f);
  assert_roughly_equal<float>(Chi2Distribution::CumulativeDistributiveInverseForProbability<float>(0, 5.f),
                     0.f);
