...
bool SumGraphController::handleEvent(Ion::Events::Event event) {
  ...
  if ((event == Ion::Events::Copy || event == Ion::Events::Cut || event == Ion::Events::Sto || event == Ion::Events::Var) &&  m_step == Step::Result) {
...
bool SumGraphController::handleLeftRightEvent(Ion::Events::Event event) {
  ...
  double newPosition = cursorNextStep(oldPosition, event == Ion::Events::Left ? OMG::HorizontalDirection::Left() : OMG::HorizontalDirection::Right());
