...
namespace Poincare
{
  ...
  template <typename T>
  Complex<T> XnorExplicitNode::computeOnComplex(const std::complex<T> c, Preferences::ComplexFormat, Preferences::AngleUnit angleUnit)
  {
    if (c.imag() != 0)
    {
  ...
  Expression XnorExplicit::shallowReduce(ExpressionNode::ReductionContext reductionContext)
  {
    ...
    Integer x = Integer::Xnor(aq, bq, cq);
