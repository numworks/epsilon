...
class Range2D {
 public:
  ...
  constexpr Range2D(float xMin = NAN, float xMax = NAN, float yMin = NAN,
                    float yMax = NAN)
      : Range2D(Range1D(xMin, xMax), Range1D(yMin, yMax)) {}
  ...
  Coordinate2D<float> center() const {
    return Coordinate2D<float>(m_x.center(), m_y.center());
  }
