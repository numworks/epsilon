function InteractiveMemoryMapViewer(chartNodeSelector, legendNodeSelector) {
  ...
  function compressedScaleFor(zone, gapRemovalRatio, equalizingRatio, rangeMin, rangeMax) {
    ...
    for (let child of sortedChildren) {
      ...
      if (gapSize > 0) {
        correctedGapSize = weightedMean(
          gapRemovalRatio,
          weightedMean(equalizingRatio, gapSize, equalizedGapSize),
          0,
        );
      } else {
        correctedGapSize = 0;
    ...
    let correctedRange = correctedLimits.map(
      (address) =>
        rangeMin + ((rangeMax - rangeMin) * (address - zone.start)) / (zone.end - zone.start),
    );
  ...
  function redraw(transitionDuration = 0) {
    ...
    maps
      ...
      .attr('transform', function (d, i) {
        return `translate(0, ${yScale(d.end)})`;
      });
    maps
      .selectAll('.zone rect')
      .transition()
    ...
    yDomAxis.transition().duration(transitionDuration).call(yAxis.scale(yScale));
  }
  ...
  this.load = function (data) {
    ...
    let cursor = svg
      ...
      .attr('width', width)
      .attr('height', '0.1');
    let cursorLegend = svg.append('text').attr('text-anchor', 'end');
    ...
    let maps = svg.selectAll('g.maps').data(data).enter().append('g').attr('class', 'maps');
    ...
    tabs.on('click', function (ev, datum) {
    ...
    let zones = maps
      .attr('transform', function (d, i) {
        return `translate(${(columnWidth + columnMargin) * i},0)`;
      })
      .selectAll('g.zone')
      .data(function (d) {
        return d.zones;
    ...
    zones
      ...
      .on('mouseover', function (element, datum) {
        ...
        legend +=
          '    <td><code>' +
          NumberToHumanBytes(datum.end - datum.start) +
          ' (' +
          (datum.end - datum.start).toLocaleString() +
          ' bytes)' +
          '</code></td>';
        legend += '  </tr>';
        legend += '  <tr>';
    ...
    zones.append('text').text((d) => d.name);
