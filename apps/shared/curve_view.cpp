...
void CurveView::drawLabelsAndGraduations(KDContext * ctx, KDRect rect, Axis axis, bool shiftOrigin, bool graduationOnly, bool fixCoordinate, KDCoordinate fixedCoordinate, KDColor backgroundColor) const {
  ...
  KDCoordinate maximalVerticalPosition = graduationOnly ? viewHeight : viewHeight - KDFont::GlyphHeight(k_font) - k_labelMargin;
  ...
  if (axis == Axis::Horizontal) {
    ...
    // We want do display at least 3 characters left of the Y axis
    KDCoordinate minimalHorizontalPosition = graduationOnly ? 0 : k_labelMargin + KDFont::GlyphWidth(k_font) * 3;
  ...
  if (axis == Axis::Vertical) {
     ...
     * labels */
    int horizontalLabelsMargin = KDFont::GlyphHeight(k_font) * 2;
    ...
    if (shouldDrawLabelAtPosition(labelValue)) {
      ...
      if (axis == Axis::Horizontal) {
        ...
        if (floatingLabels == FloatingPosition::Min) {
          ...
          position = KDPoint(position.x(), viewHeight - KDFont::GlyphHeight(k_font) - k_labelMargin);
        }
      } else {
...
void CurveView::layoutSubviews(bool force) {
  ...
  if (m_bannerView != nullptr) {
    ...
    if (!(m_bannerView->bounds() == oldFrame)) {
      const KDCoordinate dirtyHeight = std::max(oldFrame.height(), m_bannerView->bounds().height()) + KDFont::GlyphHeight(k_font) + 2 * k_labelMargin;
...
KDRect CurveView::bannerFrame() {
  ...
  if (bannerIsVisible()) {
    // Else the bannerHeight will not be properly computed
    assert(bounds().width() == Ion::Display::Width);
