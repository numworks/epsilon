...
public:
  ...
  void clear();
  Poincare::Coordinate2D<float> valueForParameter(const NewFunction * function, Poincare::Context * context, float t);
...
private:
  ...
  void setRange(NewFunction * function, float tMin, float tStep);
  int indexForParameter(const NewFunction * function, float t) const;
  Poincare::Coordinate2D<float> valuesAtIndex(const NewFunction * function, Poincare::Context * context, float t, int i);
  void pan(NewFunction * function, float newTMin);
