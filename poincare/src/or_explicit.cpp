...
namespace Poincare
{
  ...
  int OrExplicitNode::numberOfChildren() const { return OrExplicit::s_functionHelper.numberOfChildren(); }

  Layout OrExplicitNode::createLayout(Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return LayoutHelper::Prefix(OrExplicit(this), floatDisplayMode, numberOfSignificantDigits, OrExplicit::s_functionHelper.name());
  }

  int OrExplicitNode::serialize(char *buffer, int bufferSize, Preferences::PrintFloatMode floatDisplayMode, int numberOfSignificantDigits) const
  {
    return SerializationHelper::Prefix(this, buffer, bufferSize, floatDisplayMode, numberOfSignificantDigits, OrExplicit::s_functionHelper.name());
  }

  Expression OrExplicitNode::shallowReduce(ReductionContext reductionContext)
  {
    return OrExplicit(this).shallowReduce(reductionContext);
  }

  template <typename T>
  Complex<T> OrExplicitNode::computeOnComplex(const std::complex<T> c, Preferences::ComplexFormat, Preferences::AngleUnit angleUnit)
  {
    if (c.imag() != 0)
    {
