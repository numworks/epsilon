...
def random_write(flash_offset, length):
    ...
    os.system(
        "dfu-util -i 0 -a 0 -s 0x%x -D data.bin > /dev/null 2>&1"
        % (flash_base_address + flash_offset)
    )
    os.system("rm result.bin")
    os.system(
        "dfu-util -i 0 -a 0 -s 0x%x:%d -U result.bin > /dev/null 2>&1"
        % (flash_base_address, flash_offset + length + 100)
    )
