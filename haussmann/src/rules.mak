...
BASE ?= HEAD
_build_commit := $(shell $(GIT) rev-parse --verify --quiet $(BASE)^{commit} || echo error)

ifneq ($(_build_commit),$(_head_commit))

ifeq ($(_build_commit),error)
$(error Error: Invalid base branch $(BASE). Please use a valid branch name or commit hash.)
endif
...
_worktree_build:
	if [ ! -d $(_worktree_dir) ]; then \
		$(GIT) worktree add -f $(_worktree_dir) $(BASE) && \
		rm -rf $(_worktree_dir)/.git && \
		$(GIT) worktree prune; \
	fi
	$(MAKE) -C $(_worktree_dir) $(MAKECMDGOALS) BASE=
