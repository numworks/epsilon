...
void assert_interest_solves(const double* expectedValues, bool booleanParam,
                            InterestData* data) {
  // Set all parameters
  data->m_booleanParam = booleanParam;
  for (uint8_t paramIndex = 0; paramIndex < data->numberOfDoubleValues();
       paramIndex++) {
    data->setValue(paramIndex, expectedValues[paramIndex]);
  }
  // Test each parameters
  for (uint8_t paramIndex = 0; paramIndex < data->numberOfUnknowns();
       paramIndex++) {
    ...
    assert_roughly_equal(data->computeUnknownValue(), expectedValue,
                         k_precision, false);
