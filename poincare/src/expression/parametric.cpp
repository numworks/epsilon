...
Tree* ReduceSumOrProductAux(const Tree* e, const Tree* lowerBound,
  ...
  if (Variables::CanEnterOrLeaveScope(function)) {
  ...
  if (isSum && function->isMult()) {
    ...
    for (int i = 0; i < nbChildren; i++) {
      if (Variables::CanEnterOrLeaveScope(child)) {
  ...
  if (!isSum && function->isPow()) {
    ...
    assert(Variables::CanEnterOrLeaveScope(function->child(1)));
...
bool Parametric::ReduceSumOrProduct(Tree* e) {
  ...
  result->removeTree();
  // CreateReduce may have reduced the dependency into undefined
  if (e->isDep()) {
