...
App::App(Snapshot * snapshot, Poincare::Context * parentContext) :
  ...
  m_graphController(&m_graphAlternateEmptyViewController, this, &m_graphHeader, snapshot->range(), &m_store, snapshot->cursor(), snapshot->graphSelectedDotIndex(), snapshot->selectedSeriesIndex()),
