...
bool Expression::isApproximate(Context & context) const {
  return recursivelyMatches([](const Expression e, Context & context) {
        return e.type() == ExpressionNode::Type::Decimal || e.type() == ExpressionNode::Type::Float || IsMatrix(e, context) || (e.type() == ExpressionNode::Type::Symbol && !static_cast<const Symbol&>(e).isConstant() && static_cast<const Symbol&>(e).matches([](const Expression e, Context & context) { return e.isApproximate(context); }, context));
    }, context);
}
