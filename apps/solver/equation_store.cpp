...
Poincare::Layout EquationStore::exactSolutionLayoutAtIndex(int i,
                                                           bool exactLayout) {
...
void EquationStore::approximateSolve(Poincare::Context *context,
  ...
  Expression undevelopedExpression =
      modelForRecord(definedRecordAtIndex(0))
          ->standardForm(context, shouldReplaceFunctionsButNotSymbols,
  ...
  Poincare::Solver<double> solver =
      PoincareHelpers::Solver(start, end, m_variables[0], context);
  solver.stretch();
...
EquationStore::Error EquationStore::exactSolve(
  ...
  *replaceFunctionsButNotSymbols = false;
  ...
  if (m_numberOfUserVariables > 0 &&
      (firstError != Error::NoError || firstNumberOfSolutions == 0 ||
       firstNumberOfSolutions == INT_MAX)) {
...
EquationStore::Error EquationStore::privateExactSolve(
    Poincare::Context *context, bool replaceFunctionsButNotSymbols) {
  ...
  // TODO we look twice for variables but not the same, is there a way to not do
  // the same work twice?
  ...
  for (int i = 0; i < definedModelsTotal; i++) {
    Shared::ExpiringPointer<Equation> eq =
    ...
    if (m_numberOfUserVariables < Expression::k_maxNumberOfVariables) {
      const Expression eWithSymbols =
          eq->standardForm(context, true, ReductionTarget::SystemForAnalysis);
      ...
      int varCount = eWithSymbols.getVariables(
          context,
          [](const char *symbol, Poincare::Context *context) {
            return context->expressionTypeForIdentifier(symbol,
                                                        strlen(symbol)) ==
    ...
    if (simplifiedExpressions[i].isUninitialized()) {
      ...
      simplifiedExpressions[i] =
          eq->standardForm(context, replaceFunctionsButNotSymbols,
                           ReductionTarget::SystemForAnalysis);
    }
    const Expression e = simplifiedExpressions[i];
    if (e.isUninitialized() || e.type() == ExpressionNode::Type::Undefined ||
        e.recursivelyMatches(Expression::IsMatrix, context,
                             symbolicComputation)) {
  ...
  bool isLinear =
      true;  // Invalid the linear system if one equation is non-linear
  ...
  assert(nbOfDefinedModels <= k_maxNumberOfEquations);
  {
    ...
    if (isLinear) {
      m_degree = 1;
      m_type = Type::LinearSystem;
      error = resolveLinearSystem(exactSolutions, exactSolutionsApproximations,
                                  coefficients, constants, context);
    }
  ...
  if (!isLinear) {
    ...
    Expression
        polynomialCoefficients[Expression::k_maxNumberOfPolynomialCoefficients];
    m_degree = simplifiedExpressions[0].getPolynomialReducedCoefficients(
        m_variables[0], polynomialCoefficients, context,
        updatedComplexFormat(context), angleUnit, unitFormat,
        symbolicComputation);
    if (m_degree == 2 || m_degree == 3) {
      // Polynomial degree <= 3
      m_type = Type::PolynomialMonovariable;
      error = oneDimensionalPolynomialSolve(exactSolutions,
                                            exactSolutionsApproximations,
                                            polynomialCoefficients, context);
    } else {
  ...
  int solutionIndex = 0;
  int initialNumberOfSolutions =
      m_numberOfSolutions <= k_maxNumberOfExactSolutions ? m_numberOfSolutions
                                                         : -1;
  ...
  for (int i = 0; i < initialNumberOfSolutions; i++) {
    ...
    if (exactSolutionsApproximations[i].type() ==
        ExpressionNode::Type::Nonreal) {
      // Discard nonreal solutions.
      m_numberOfSolutions--;
      continue;
    } else if (exactSolutionsApproximations[i].type() ==
    ...
    m_exactSolutionApproximateLayouts[solutionIndex] =
        PoincareHelpers::CreateLayout(exactSolutionsApproximations[i], context);
    if (ExpressionDisplayPermissions::ShouldNeverDisplayExactOutput(
            exactSolutions[i], context)) {
      ...
      m_exactSolutionExactLayouts[solutionIndex] =
          PoincareHelpers::CreateLayout(exactSolutions[i], context);
      char exactBuffer[::Constant::MaxSerializedExpressionSize];
      char approximateBuffer[::Constant::MaxSerializedExpressionSize];
      m_exactSolutionExactLayouts[solutionIndex].serializeForParsing(
          exactBuffer, ::Constant::MaxSerializedExpressionSize);
      m_exactSolutionApproximateLayouts[solutionIndex].serializeForParsing(
          approximateBuffer, ::Constant::MaxSerializedExpressionSize);
      m_exactSolutionIdentity[solutionIndex] =
          strcmp(exactBuffer, approximateBuffer) == 0;
      if (!m_exactSolutionIdentity[solutionIndex]) {
        /* We parse approximateBuffer instead of using directly
         * exactSolutionsApproximations[i] because
         * exactSolutionsApproximations[i] is Float and
         * ExactAndApproximateExpressionsAreEqual is always false for Float.
         * Indeed we Parse like in Calculation (see
         * Calculation::approximateOutput) */
        m_exactSolutionEquality[solutionIndex] =
            Expression::ExactAndApproximateExpressionsAreEqual(
                exactSolutions[i],
                Expression::Parse(approximateBuffer, nullptr));
      }
    }
    solutionIndex++;
  }
...
EquationStore::Error EquationStore::resolveLinearSystem(
    Expression exactSolutions[k_maxNumberOfExactSolutions],
    Expression exactSolutionsApproximations[k_maxNumberOfExactSolutions],
    Expression coefficients[k_maxNumberOfEquations]
                           [Expression::k_maxNumberOfVariables],
    Expression constants[k_maxNumberOfEquations], Context *context) {
  Preferences::AngleUnit angleUnit =
      Preferences::sharedPreferences->angleUnit();
  Preferences::UnitFormat unitFormat =
      GlobalPreferences::sharedGlobalPreferences->unitFormat();
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      Ab.addChildAtIndexInPlace(coefficients[i][j], Ab.numberOfChildren(),
                                Ab.numberOfChildren());
    }
    Ab.addChildAtIndexInPlace(constants[i], Ab.numberOfChildren(),
                              Ab.numberOfChildren());
  }
  Ab.setDimensions(m, n + 1);

  // Compute the rank of (A | b)
  int rankAb = Ab.rank(context, updatedComplexFormat(context), angleUnit,
                       unitFormat, true);
  if (rankAb == -1) {
    return Error::EquationUndefined;
  }
  ...
  if (m_numberOfSolutions > 0) {
    ...
    if (rankAb == n && n > 0) {
      ...
      for (int i = 0; i < m_numberOfSolutions; i++) {
        ...
        Ab.matrixChild(i, n).cloneAndSimplifyAndApproximate(
            exactSolutions + i, exactSolutionsApproximations + i, context,
            updatedComplexFormat(context), angleUnit, unitFormat);
      }
    }
  }
...
EquationStore::Error EquationStore::oneDimensionalPolynomialSolve(
    Expression exactSolutions[k_maxNumberOfExactSolutions],
    Expression exactSolutionsApproximations[k_maxNumberOfExactSolutions],
  ...
  ReductionContext reductionContext(
      context, updatedComplexFormat(context),
      Poincare::Preferences::sharedPreferences->angleUnit(),
      Preferences::UnitFormat::Metric, ReductionTarget::User);
  if (m_degree == 2) {
    m_numberOfSolutions = Poincare::Polynomial::QuadraticPolynomialRoots(
        coefficients[2], coefficients[1], coefficients[0], exactSolutions,
        exactSolutions + 1, &delta, reductionContext);
  } else {
    assert(m_degree == 3);
    m_numberOfSolutions = Poincare::Polynomial::CubicPolynomialRoots(
        coefficients[3], coefficients[2], coefficients[1], coefficients[0],
        exactSolutions, exactSolutions + 1, exactSolutions + 2, &delta,
        reductionContext, &solutionsAreApproximated);
  }
  ...
  for (int i = 0; i < m_numberOfSolutions; i++) {
    if (solutionsAreApproximated) {
      ...
      exactSolutions[i] = Expression();
      exactSolution.cloneAndSimplifyAndApproximate(
          exactSolutions + i, exactSolutionsApproximations + i, context,
...
void EquationStore::tidySolution(char *treePoolCursor) {
  for (int i = 0; i < k_maxNumberOfExactSolutions; i++) {
    if (treePoolCursor == nullptr ||
        m_exactSolutionExactLayouts[i].isDownstreamOf(treePoolCursor)) {
      m_exactSolutionExactLayouts[i] = Layout();
      assert(
          treePoolCursor == nullptr ||
          m_exactSolutionApproximateLayouts[i].isDownstreamOf(treePoolCursor));
...
bool EquationStore::isExplicitlyComplex(Context *context) {
  ...
  for (int i = 0; i < definedModelsTotal; i++) {
    ...
    if (modelForRecord(definedRecordAtIndex(i))
            ->containsIComplex(
                context, m_userVariablesUsed
                             ? SymbolicComputation::
