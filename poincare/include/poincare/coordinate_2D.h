...
class Coordinate2D final {
 public:
  ...
  operator Coordinate2D<C>() {
    return Coordinate2D<C>(static_cast<C>(m_x1), static_cast<C>(m_x2));
  }
  ...
  bool x1IsIn(T start, T end, bool includeStart = false,
              bool includeEnd = false) {
    return coordinateIsIn(true, start, end, includeStart, includeEnd);
  }
  bool x2IsIn(T start, T end, bool includeStart = false,
              bool includeEnd = false) {
    return coordinateIsIn(false, start, end, includeStart, includeEnd);
  }

 private:
  bool coordinateIsIn(bool x1, T start, T end, bool includeStart,
                      bool includeEnd) {
    T var = x1 ? m_x1 : m_x2;
