...
constexpr static uint8_t topLeftCurve[ParenthesisLayoutNode::k_curveHeight][ParenthesisLayoutNode::k_curveWidth] = {
...
constexpr static uint8_t bottomLeftCurve[ParenthesisLayoutNode::k_curveHeight][ParenthesisLayoutNode::k_curveWidth] = {
  {0x11, 0xEE, 0xFF, 0xFF, 0xFF},
  {0x45, 0xBE, 0xFF, 0xFF, 0xFF},
  {0xA9, 0x5A, 0xFF, 0xFF, 0xFF},
  {0xFF, 0x49, 0xB6, 0xFF, 0xFF},
...
constexpr static uint8_t topRightCurve[ParenthesisLayoutNode::k_curveHeight][ParenthesisLayoutNode::k_curveWidth] = {
...
constexpr static uint8_t bottomRightCurve[ParenthesisLayoutNode::k_curveHeight][ParenthesisLayoutNode::k_curveWidth] = {
  {0xFF, 0xFF, 0xFF, 0xEE, 0x11},
  {0xFF, 0xFF, 0xFF, 0xBE, 0x45},
  {0xFF, 0xFF, 0xFF, 0x5A, 0xA9},
  {0xFF, 0xFF, 0xB6, 0x49, 0xFF},
...
void ParenthesisLayoutNode::RenderWithChildHeight(bool left, KDCoordinate childHeight, KDContext * ctx, KDPoint p, KDColor expressionColor, KDColor backgroundColor) {
  ...
  frame = KDRect(k_widthMargin, parenthesisHeight - k_curveHeight - k_verticalMargin, k_curveWidth, k_curveHeight);
  ...
  KDCoordinate barX = k_widthMargin + (left ? 0 : k_curveWidth - k_lineThickness);
  KDCoordinate barHeight = parenthesisHeight - 2 * (k_curveHeight + k_verticalMargin);
  ctx->fillRect(KDRect(barX, k_curveHeight + k_verticalMargin, k_lineThickness, barHeight).translatedBy(p), expressionColor);
}
