...
public:
  CalculusColumnParameterController(I18n::Message hideMessage, ValuesController * valuesController);

  bool handleEvent(Ion::Events::Event event) override;
  int numberOfRows() const override { return k_totalNumberOfCell; }
  Escher::HighlightCell * reusableCell(int index, int type) override { assert(index == 0); return &m_hideColumn; }
  void setRecord(Ion::Storage::Record record) { m_record = record; }

protected:
  virtual void hideCalculusColumn() = 0;
  Shared::ColumnNameHelper * columnNameHelper() override { return m_valuesController; }
