...
class SimpleInteractiveCurveViewController : public ZoomCurveViewController,
 ...
 protected:
  ...
  virtual float cursorRightMarginRatio() const {
    return 0.04f;
  }  // (cursorWidth/2)/(graphViewWidth-1)
  virtual float cursorLeftMarginRatio() const {
    return 0.04f;
  }  // (cursorWidth/2)/(graphViewWidth-1)
  virtual float cursorTopMarginRatio() const {
    return 0.07f;
  }  // (cursorHeight/2)/(graphViewHeight-1)
  virtual float cursorBottomMarginRatio()
      const = 0;  // (cursorHeight/2+bannerHeight)/(graphViewHeight-1)
  ...
  virtual bool moveCursorHorizontally(OMG::HorizontalDirection direction,
                                      int scrollSpeed = 1) {
    return false;
  }
