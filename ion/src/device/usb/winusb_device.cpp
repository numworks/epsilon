...
static const struct WinUSBDevice::OSStringDescriptor osStringDescriptor = {
...
static const struct WinUSBDevice::OSExtendedCompatIDHeader osExtendedCompatIDHeader = {
...
static const struct WinUSBDevice::OSExtendedCompatIDFunction osExtendedCompatIDFunction = {
...
static const struct WinUSBDevice::OSExtendedPropertiesHeader osExtendedPropertiesHeader = {
  .dwLength = 0,
...
static const struct WinUSBDevice::OSExtendedPropertiesGUIDFunction osExtendedPropertiesGUIDFunction = {
...
bool WinUSBDevice::buildDataToSend(SetupData *setupData, uint8_t ** bufferToSend, uint16_t * bufferToSendLength, uint8_t * bufferForWriting) {
  if (setupData->bRequest() == USB_REQ_GET_DESCRIPTOR) {
    if (setupData->descriptorType() == USB_DT_STRING) {
      assert(setupData->descriptorIndex() == USB_WINDOWS_OS_STRING_DESCRIPTOR_INDEX);
...
uint16_t WinUSBDevice::buildExtendedCompatIDDescriptor(uint8_t * buffer, uint16_t length) {
  /* Copy the header */
...
uint16_t WinUSBDevice::buildExtendedPropertiesDescriptor(uint8_t * buffer, uint16_t length) {
