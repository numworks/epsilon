...
void testPersistingByte(uint8_t expectedValue, uint8_t index) {
  uint8_t observedValue = PersistingBytes::read(index);
  if (observedValue != expectedValue) {
    constexpr size_t bufferSize = sizeof("At 255 : 255 instead of 255.");
    char buffer[bufferSize];
    Poincare::Print::customPrintf(buffer, bufferSize, "At %i : %i instead of %i.", index, observedValue, expectedValue);
...
QUIZ_CASE(ion_persisting_bytes) {
  ...
  for (uint8_t i = 1; i < k_numberOfTests; i++) {
    if (pressToTest[i] != pressToTest[i-1]) {
      testPersistingByte(pressToTest[i-1], 0);
      PersistingBytes::write(pressToTest[i], 0);
    }
    if (examModes[i] != examModes[i-1]) {
      testPersistingByte(examModes[i-1], 1);
      PersistingBytes::write(examModes[i], 1);
    }
  }
  testPersistingByte(0, 0);
  testPersistingByte(0, 1);
}
