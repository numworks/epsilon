...
QUIZ_CASE(omg_memory_insert_simple_swap) {
  ...
  Memory::Rotate(&buf[0], &buf[1], 1);
  quiz_assert(buf[0] == 1);
  quiz_assert(buf[1] == 0);
  quiz_assert(buf[2] == 2);

  Memory::Rotate(&buf[2], &buf[0], 1);
  quiz_assert(buf[0] == 0);
  quiz_assert(buf[1] == 1);
  quiz_assert(buf[2] == 2);
}
...
static inline void test_rotate(uint32_t buf[], size_t bufSize, size_t dstIndex,
  ...
  Memory::Rotate(&buf[dstIndex], &buf[srcIndex], len);
